...
/**
 * AI-based automatic score assignment
 * Analyzes event name and assigns intelligent scores based on perceived value
 */
function calculateAIScore(eventName) {
  const name = eventName.toLowerCase();
  
  // High-value conversion events (15-25 points)
  if (name.includes('purchase') || name.includes('checkout') || name.includes('payment')) {
    return 25;
  }
  if (name.includes('signup') || name.includes('register') || name.includes('subscribe')) {
    return 20;
  }
  if (name.includes('lead') || name.includes('contact') || name.includes('quote')) {
    return 18;
  }
  if (name.includes('form_submit') || name.includes('form_complete')) {
    return 15;
  }
  
  // Medium-value engagement events (8-12 points)
  if (name.includes('download') || name.includes('pdf')) {
    return 12;
  }
  if (name.includes('video') && (name.includes('play') || name.includes('watch'))) {
    return 10;
  }
  if (name.includes('add_to_cart') || name.includes('cart_add')) {
    return 10;
  }
  if (name.includes('share') || name.includes('social')) {
    return 8;
  }
  ...
  // Low-value engagement events (2-5 points)
  if (name.includes('click') || name.includes('button')) {
    return 3;
  }
  if (name.includes('scroll')) {
    return 2;
  }
  if (name.includes('engagement') || name.includes('interact')) {
    return 4;
  }
  
  // Page views and basic navigation (1 point)
  if (name.includes('page_view') || name.includes('pageview')) {
    return 1;
  }
  if (name.includes('page_load') || name.includes('load')) {
    return 1;
  }
  
  // Negative value events (-5 to -1 points)
  if (name.includes('unsubscribe') || name.includes('opt_out')) {
    return -5;
  }
  if (name.includes('error') || name.includes('404')) {
    return -2;
  }
  if (name.includes('bounce') || name.includes('exit')) {
    return -1;
  }
  
  // Default: medium-low value for unknown events
  return 5;
}
...
/**
 * Calculate score for an event based on active rules or AI
 * If no rule exists, creates one automatically using AI scoring
 * @param {string} siteId - Site UUID
 * @param {string} eventName - Event name
...
async function calculateEventScore(siteId, eventName) {
  try {
    ...
    if (result.rows.length > 0) {
      return result.rows[0].score_value;
    }
    
    // No rule exists - use AI to calculate score
    const aiScore = calculateAIScore(eventName);
    
    // Auto-create rule for future use
    try {
      await db.query(
        `INSERT INTO event_scoring_rules (id, site_id, event_name, score_value, description, active)
        ...
        [
          ...
          eventName,
          aiScore,
          'Auto-generated by AI scoring engine'
        ]
      );
      console.log(`[AI Scoring] Auto-created rule for ${eventName}: ${aiScore} points`);
    } catch (insertErr) {
      console.error('Error auto-creating scoring rule:', insertErr);
      // Continue with AI score even if rule creation fails
    }
    
    return aiScore;
  } catch (error) {
    console.error('Error calculating event score:', error);
    // Fallback to AI if database error
    return calculateAIScore(eventName);
  }
}
